...
local function InitSPKZCmds()
		...
		CreateConVar( "SPKZ_DKS_TeamMode", "0", FCVAR_ARCHIVE, "If 0, Ignore Friendly Fire Checking" )
...
local function CheckFF(ply1,ply2) --Return TRUE if is friendly fire
	if not cvars.Bool("SPKZ_DKS_TeamMode") then return false end
...
local function SPKZ_EntityTakeDamage(ent, dmginfo)
...
local function SPKZ_PlayerSilentDeath(ply)
...
-- override base function lel
local function SPKZ_PostGamemodeLoaded()
...
DotaKillStreak.PlayerDeath = function( victim, inflictor, attacker )
	SPKZ_PlayerKilled(victim, inflictor, attacker)
end

DotaKillStreak.ResetAll = function()
	SPKZ_ResetAll()
end

DotaKillStreak.GetKill = function(ply)
	if not IsValid(ply) then return end
	return ply:GetNWInt("SPKZ_CSKillStreak",1)
end

DotaKillStreak.SetKill = function(ply,newvalue)
	if not IsValid(ply) then return end
	if not isnumber(newvalue) then
		newvalue = tonumber(newvalue)
	end
	ply:SetNWInt("SPKZ_CSKillStreak",newvalue)
end